<template>
	<div class="flex-1 coupon">
		<div class="box-con bg-white">
			<p class="com-title bg-pri">{{$t('user.voucher.tit')}}</p>
			<div class="con1">
				<div class="row bor-b-ccc pb-20">
					<div class="col-sm-4 item">
						<div class="item-con item-con1">
							<p>{{$t('user.gift.total')}}（{{ this.sumcount }}）</p>
							<div class="text-center f-24">{{$t('user.gift.value')}}：¥{{this.dTotalGifts}}</div>
						</div>
					</div>
					<div class="col-sm-4 item">
				...
				<el-tabs type="border-card" class="coupon-tabs" @tab-click="onTabClick">
					<el-tab-pane class="tab-item" name="0">
						<span slot="label"><i class="el-icon-date"></i>{{$t('user.voucher.tit')}}</span>
						<div class="tab-con">
							<div class="row">
	...
	export default {
		data() {
			return {
				...
				pageNum: 1, //当前页数
				numPerPage: 10, //每页数量
				status: 0, //选项卡下标0表示默认，1表示已使用，-1表示未使用
		...
		methods: {
			select(status, pageNum, numPerPage) {
				this.$http.get(this.$root.urlPath.MJK + '/account/others?role=1&function=cashCoupon&pageNum=' + pageNum + '&numPerPage=' + numPerPage + '&status=' + status).then(function(res) {
					if(res.body.success) {
						if(res.data.data.mnCurrencyVOS != null && res.data.data.mnCurrencyVOS.length > 0) {
							//						console.log("资金管理，代金券",res.data.data)
