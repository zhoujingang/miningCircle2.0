		...
		<div class="vouc_sec">
			...
			<div class="con withdraw">
				...
				<div class="pay_con">
					...
					<label class="pay clearfix f-18">
						<span>资金密码:</span><input autocomplete="new-password" type="password" maxlength="25" v-model="pwd" class="ipt-text"/>
					</label>
...
<script>
	...
	export default{
		...
		created(){
			...
			this.$http.get(this.$root.urlPath.MJK + '/rechargecash/checkBankCard').then(function(res){
				if(res.body.success){
		...
		methods: {
			showCaptcha() {//是否显示验证码
				this.$http.get(this.$root.urlPath.MJK+ '/rechargecash/showVerificationCapt?status=1').then(function(res){
					if(res.body.success){
			...
			withdraw() {
				...
				if(this.numError||this.pwdError||this.codeError){
					...
					this.$http.get(this.$root.urlPath.MJK+ '/rechargecash/withdrawals?status=1&moneyVal=' + this.money + '&pwd=' + this.pwd + '&captcha='+ this.code).then(function(res){
						if(res.body.success){
							this.$message({
								showClose: true,
								message: '恭喜您，提现成功',
