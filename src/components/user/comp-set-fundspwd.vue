<template>
	...
	<div class="flex-1 act-sec box-con">
		<p class="com-title bg-pri">修改企业资金密码</p>
						...
						<el-row>
							<el-col :span="20">
								<el-form-item label="短信验证码:" prop="msgCode">
									<el-input v-model="ruleForm1.msgCode"  placeholder="请输入验证码"></el-input>
								</el-form-item>
							</el-col>
							<el-col :span="4">
								<!--<el-form-item label="">-->
						...
						<el-row>
							<el-col :span="20">
								<el-form-item label="图形验证码:" prop="captcha">
									<el-input v-model="ruleForm1.captcha"  placeholder="请输入验证码"></el-input>
								</el-form-item>
							</el-col>
							<el-col :span="4">
								<!--<el-form-item label="">-->
...
//					oldPwd: '',
		...
		methods: {
			submitForm :function(formName){
				this.$refs[formName].validate((valid) => {
					if(valid){
						...
						this.$http.get(this.$root.urlPath.MJK + '/account/mobile?type=2',{params:params}).then(function(res){
							this.reset_code();
							if(res.body.success){
		        				this.step = '2';
		        			}else{
			...
			submitForm2 :function(formName){
				this.$refs[formName].validate((valid) => {
					if(valid){
						...
						this.$http.post(this.$root.urlPath.MJK + '/userCompany/saveOrUpdateCompanyCapitalPsw',params).then(function(res){
							if(res.body.success){
								this.step = '3';
							}else{
								this.$message({
			...
			get_code() {
        		...
        		this.show_time = true;
//      		this.$http.get(this.$root.urlPath.MJK+'/common/sms?type=7&mobile='+this.ruleForm1.mobile).then(function(res){
//      			console.log(res);
//      		},function(res){
//      			
//      		})
        		this.$http.get(this.$root.urlPath.MJK+'/common/getSmsPre?type=7&mobile='+this.ruleForm1.mobile).then(function(res){
        			if (res.body.success) {
			...
			getCaptcha(){
        		this.$http.get(this.$root.urlPath.MJK+'/common/getCaptchaPre?type=8').then(function (res) {
	        		if (res.body.success) {
        				this.mc_cap = res.body.data         				
						this.imgCode = this.$root.urlPath.MJK+'/common/captcha?type=8&MC_CAPTCHA=' + this.mc_cap
	        		}
	        	})
        	},
			reset_code() {//刷新验证码
