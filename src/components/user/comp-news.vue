		...
		<div class="box-con bg-white">
			...
			<div class="con1">
				...
				<div class="clearfix pt-20 pb-20 pl-20">
					...
					<div class="pull-left">
						<button class="del btn" @click="Delete">{{$t('user.message.delete')}}</button>
					</div>
					<div class="pull-left">
						<button class="read btn" @click="Readed('Click')">{{$t('user.message.marked_read')}}</button>
					</div>
	...
	export default {
		...
		methods: {
			load(pageNum, numPerPage) { //加载数据
				this.$http.get(this.$root.urlPath.MJK + '/note/noteList?cat=1&pageNum=' + pageNum + '&numPerPage=' + numPerPage).then(function(res) {
...
//								this.list[i].ischeck = false;
			...
			Delete() { //删除
				const id = this.checkboxModel.join(',');
				if(!id) {
					return false;
				}
				this.loading = true;
				this.$http.get(this.$root.urlPath.MJK + '/note/noteOperate?function=noteDelete&status=1&ids=' + id).then(function(res) {
					if(res.body.success) {
						this.checkboxModel = [];						
						this.allCheck = false;
						this.load(this.pageNum, this.numPerPage);
			...
			Readed() { //已读
				...
				this.$http.get(this.$root.urlPath.MJK + '/note/noteOperate?function=noteClick&status=1&ids=' + id).then(function(res) {
					if(res.body.success) {
						this.load(this.pageNum, this.numPerPage);
					}
				}, function() {
