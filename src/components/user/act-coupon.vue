	...
	export default {
		...
		methods: {
			select(status, pageNum, numPerPage) {
				this.$http.get(this.$root.urlPath.MJK + '/account/others?role=0&function=cashCoupon&pageNum=' + pageNum + '&numPerPage=' + numPerPage + '&status=' + status).then(function(res) {
					if(res.body.success) {
