<template>
	<div class="flex-1 funds">
		<div class="box-con bg-white">
			...
			<div class="clearfix">
				<router-link to="/comp/comp_funds/withdraw" class="draw btn">{{$t('user.index.withdraw')}}</router-link>
				<router-link to="/comp/comp_funds/recharge" class="pay btn">{{$t('user.index.recharge')}}</router-link>
			</div>
		</div>
		<div class="box-con bg-white">
			...
			<div class="history-out">
				<p class="f-16 pb-15"><strong>{{$t('user.funds.type')}}：</strong>
					<span v-for="(item,index) in transactionArr" :class="index==isActive?'col-pri':''" @click="changeClass(index,item.type)"> {{item.name}}</span>
				</p>
				<p class="f-16"><strong>{{$t('user.funds.time_ran')}}：</strong>
					<span v-for="(item,index) in timeArr" class="a" :class="index==isActive2?'col-pri':''" @click="changeClass2(index,item.type)"> {{item.name}}</span>
				</p>
				<table class="full-w f-14">
					<tr>
						<th style="width: 25%;">{{$t('user.funds.time')}}</th>
						<th style="width: 18%;">{{$t('user.funds.type')}}</th>
						<th style="width: 17%;">{{$t('user.funds.amount')}}</th>
						<th style="width: 15%;">{{$t('comp.funds.status')}}</th>
						<th style="width: 25%;">{{$t('user.funds.explain')}}</th>
					</tr>
	...
	export default {
		data() {
			return {
				...
				codeMsg: [],
				accountType: {
					"1": this.$t('comp.funds.income'),
					"-1": this.$t('comp.funds.expenditure'),
					"0": this.$t('comp.funds.freeze'),
				}
			}
		},
		created() {
			this.$http.get(this.$root.urlPath.MJK + '/account/account?status=1').then(function(res) {
				if(res.body.success) {
		...
		methods: {
			...
			select(transactionType, timeFrame, pageNum, numPerPage) {
				this.$http.get(this.$root.urlPath.MJK + '/account/accountList?status=1&transactionType=' + transactionType + '&timeFrame=' + timeFrame + '&pageNum=' + pageNum + '&numPerPage=' + numPerPage).then(function(res) {
					if(res.body.success) {
						//					console.log("资金管理，我的资金,资金记录",res.data.data)
						if(res.data.data.accountDataVOS != null && res.data.data.accountDataVOS != null && res.data.data.accountDataVOS.length > 0) {
